...
namespace Detail
{
	...
	template<typename State, typename Event, typename T, typename... Ts>
	...
	template<typename State, typename Event, typename Ttransitions>
	...
	{
	public:
		using States = typename UniqueTypes<StatesTuple<Ts...>>::Type;
	...
	private:
		...
		static void truTransit(States& states, size_t idx, Event&& e, size_t& nextIdx)
		{
			if(Idx != idx)
				return;
			using Tr = typename GetTransition<std::tuple_element_t<Idx, States>, Event, Transitions>::Type;
		...
		{
			size_t nextIdx = 0;
			((truTransit<I>(states, idx, std::move(e), nextIdx)), ...);
			return nextIdx;
		}
	};
}
