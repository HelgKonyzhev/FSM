...
namespace FSM
{
	namespace Detail
	{
		...
		struct Transition
		{
			static_assert(Detail::HasParenthesisConstRef<C, FS, E, TS>::value || Detail::HasParenthesisMove<C, FS, E, TS>::value,
			...
			using From = Detail::StateWrapper<FS>;
			using To = Detail::StateWrapper<TS>;
			using Event = E;
			using Condition = C;
			...
			constexpr static size_t transit(States& states, Event&& e)
			{
				...
				else
				{
					...
					return Detail::TypeIndex<From, States>::value;
				}
			}
		};
	}
